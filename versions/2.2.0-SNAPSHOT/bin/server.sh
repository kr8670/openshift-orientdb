...
exec "$JAVA" $JAVA_OPTS $MAXHEAP $JAVA_OPTS_SCRIPT $ORIENTDB_SETTINGS $MAXDISKCACHE -Djava.util.logging.config.file="$LOG_FILE" -Dorientdb.config.file="$CONFIG_FILE" -Dorientdb.www.path="$WWW_PATH" -Dorientdb.build.number="UNKNOWN@r${buildNumber}; 2015-08-15 12:41:34+0000" -cp "$ORIENTDB_HOME/lib/orientdb-server-2.2.0-SNAPSHOT.jar:$ORIENTDB_HOME/lib/*" $* com.orientechnologies.orient.server.OServerMain
